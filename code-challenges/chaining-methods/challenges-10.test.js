...
const createServer = () => {
  ...
  app.get('/hello', helloThere)
  function helloThere(request, response){
    response.send('hello there');
  }
  app.get('/aboutme', aboutMe)
  function aboutMe(request, response){
    response.send('I am potato');
  }
  app.get('/favoritefoods', foodsFav)
  function foodsFav(request, response){
    let food = ['banana', 'apple']
    response.send(food);
  }
...
const count = (target, input) => {
  let start = 0;
  input.map(insideArr => {
    insideArr.map(insideinsieArr =>{
      if(insideinsieArr === target){
        start++
      }
    });
  })
...
const totalSum = (input) => {
  input.reduce(insideArr =>{
    insideArr.reduce((a, b) => a + b, 0)
  })
  return insideArr;
}
