...
const toTitleCase = (arr) => {
  let letter =  arr.map(str => {
    let first = str.substring(0, 1);
    let front = (first).toUpperCase();
    let last = str.replace(str.substring(0, 1), front);
    return last
  });
  return letter;
}
...
// chance helped create this bad boy
let biggerThanLuke = (arr) => arr.filter(bigStars => parseInt(bigStars.mass) > 78).map(starsMass => starsMass.name).join().replace( ',', ' - ');
...
const sortBy = (property, arr) => {
  let newArr = [];
  arr.filter(moneyFilter => {
    newArr.push(moneyFilter.price);
