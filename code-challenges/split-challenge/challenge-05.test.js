...
const templateWithJQuery = () => {
  for (let i = 0; i < starWarsPeople.length; i++){
    const sectionClone = $('#template').clone();
    sectionClone.find($('h2').text(starWarsPeople[0].name));
    sectionClone.find($('h3').text(starWarsPeople[1].height));
    sectionClone.find($('p').text(starWarsPeople[2].eye_color));
...
const listFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach( singleIngredient =>{
    let space = singleIngredient.indexOf(' ');
    singleIngredient = singleIngredient.slice(space+1)
    space = singleIngredient.indexOf(' ');
    singleIngredient = singleIngredient.slice(space+1)
    result.push(singleIngredient);
  })
  return result;
};
